...
{	
  ...
  for(int n=0; n<3000; n++){
	...
	if(buses[j].Ni.size() != 0 && buses[j].Ns.size() != 0){
      ...
      for(int j=0; j<NE; j++){
	if(estaciones[j].N() != 0){
	  Gillespie_estaciones(estaciones[j],inf_net,exposed,j,t,id_NP,count_NP,g_seed.r(),g_seed.r(),g_seed.r());
	}
      }
    ...
    fout.open("Rho_1.csv", std::ofstream::app);
