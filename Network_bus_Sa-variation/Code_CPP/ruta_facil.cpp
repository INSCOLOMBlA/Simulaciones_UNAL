...
{	
  ...
  for(int n=0; n<10; n++){
    ...
    for(int i=0; i<estaciones[the_station].Ns.size(); i++){
      if(ran_net.r() < prob_net){inf_net.push_back(estaciones[the_station].Ns[i]);}
    }
    ...
    //Genero la dinámica 
    for(double t=0; t<T/2; t+=7){
      
      //Hago el Gillespie en cada uno de los buses y estaciones
      for(int j=0; j<NB; j++){
	if(buses[j].Ni.size() != 0 && buses[j].Ns.size() != 0){
	  Gillespie_buses(buses[j],inf_net,exposed,j,t,g_seed.r(),g_seed.r(),g_seed.r());
	}
      }
      for(int j=0; j<NE; j++){
	if(estaciones[j].N() != 0){
	  Gillespie_estaciones(estaciones[j],inf_net,exposed,j,t,id_NP,count_NP,g_seed.r(),g_seed.r(),g_seed.r(),Sa1,Sa1);
	}
      }
	...
	for(int j=0; j<NB; j++){
	  if(aux_buses[j].Ni.size() != 0){// Si esto es distinto de cero, es porque hay un infectado.
	    ...
	    for(int k=0; k<aux_buses[j].Ns.size(); k++){
	      if(ran_net.r() < prob_net){inf_net.push_back(aux_buses[j].Ns[k]);}
	    }
	    //Agrego expuestos al vector.
	    for(int k=0; k<aux_buses[j].Ne.size(); k++){
	      if(ran_net.r() < prob_net){inf_net.push_back(aux_buses[j].Ne[k]);}
	    }
	...
	for(int j=0; j<NE; j++){
	  if(estaciones[j].Ni.size() != 0){// Si esto es distinto de cero, es porque hay un infectado.
	    ...
	    for(int k=0; k<estaciones[j].Ns.size(); k++){
	      if(ran_net.r() < prob_net){inf_net.push_back(estaciones[j].Ns[k]);}
	    }	
	    //Agrego expuestos al vector.
	    for(int k=0; k<estaciones[j].Ne.size(); k++){
	      if(ran_net.r() < prob_net){inf_net.push_back(estaciones[j].Ne[k]);}
	    }
	    //Rompo el ciclo, pues ya encontré donde estaba.
	    break;
	  }
	...
	buses[(j+1)%NB].Ns = aux_buses[j].Ns;
      ...
      aux_sum = 0;
      for(int j=0; j<NB; j++){aux_sum += buses[j].N();}
      for(int j=0; j<NE; j++){aux_sum += estaciones[j].N();}
      sum.push_back(aux_sum);
      
      //std::cout << "Número de conectados al infectado: " << inf_net.size() << std::endl;
    }

    //Genero la dinámica 
    for(double t=T/2; t<T; t+=7){
	...
	if(buses[j].Ni.size() != 0 && buses[j].Ns.size() != 0){
      ...
      for(int j=0; j<NE; j++){
	if(estaciones[j].N() != 0){
	  Gillespie_estaciones(estaciones[j],inf_net,exposed,j,t,id_NP,count_NP,g_seed.r(),g_seed.r(),g_seed.r(),Sa2,Sa2);
	}
      }
    ...
    //Hallo el promedio de personas adentro del sistema.
    prom = 0;
    for(int i=0; i<sum.size(); i++){prom += sum[i];}
    prom /= sum.size();

    //Hallo el sigma de las personas dentro del sistema.
    sigma = 0;
    for(int i=0; i<sum.size(); i++){sigma += (sum[i]-prom)*(sum[i]-prom);}
    sigma /= sum.size();
    ...
    fout.open("Sa_max_min", std::ofstream::app);
    fout << n+1 << '\t' << exposed << '\t' << prom << '\t' << sigma << std::endl;
    fout.close();
